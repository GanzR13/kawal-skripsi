...
<template>
	<div
		class="relative w-full h-full font-sans text-slate-900 dark:text-slate-50 selection:bg-emerald-500/30 selection:text-emerald-900 dark:selection:text-emerald-200">
		<transition name="fade">
			<div
				v-if="isShowLoading"
				class="fixed inset-0 z-9999 flex items-center justify-center px-4 bg-slate-950/90 backdrop-blur-md">
				<div
					class="bg-slate-900 border border-slate-700/50 p-8 rounded-3xl shadow-2xl flex flex-col items-center text-center max-w-sm w-full relative overflow-hidden">
					<div
						class="absolute top-0 w-32 h-32 bg-emerald-500/10 blur-[50px] rounded-full"></div>

					<div class="relative mb-6 z-10 animate-float-toga">
						<GraduationCap
							class="w-16 h-16 text-emerald-400 drop-shadow-[0_0_10px_rgba(52,211,153,0.3)]" />
						<Sparkles
							class="absolute -top-1 -right-1 w-4 h-4 text-cyan-300 animate-pulse-slow" />
					</div>

					<h2
						class="text-xl font-bold text-white mb-2 z-10 transition-colors duration-300">
						{{ teksLoading }}
					</h2>
					...
					<div
						class="w-full h-2 bg-slate-800 rounded-full overflow-hidden z-10">
						<div
							class="h-full bg-linear-to-r from-emerald-500 to-cyan-400 rounded-full transition-all duration-300 ease-out"
							:style="{ width: progressLoading + '%' }"></div>
					</div>
		...
		<div
			class="flex h-screen bg-slate-50 dark:bg-slate-900 overflow-hidden transition-colors duration-300">
			<transition name="fade">
				<div
					v-if="isMobileMenuBuka && route.path !== '/'"
					@click="isMobileMenuBuka = false"
			...
			<main class="flex-1 h-screen overflow-y-auto w-full relative">
				<header
					...
					<div class="flex items-center">
						...
						<h1 class="text-lg font-bold text-slate-800 dark:text-white">
							Kawal<span class="text-emerald-500">Skripsi</span>
						</h1>
					</div>
				...
				<div
					:class="
						route.path === '/'
							? 'w-full h-full'
							: 'p-4 md:p-8 lg:p-10 max-w-7xl mx-auto'
					">
